...
import com.group6.HaUISocialMedia_server.dto.RelationshipDto;
import com.group6.HaUISocialMedia_server.dto.SearchObject;
import com.group6.HaUISocialMedia_server.dto.UserDto;
import com.group6.HaUISocialMedia_server.entity.*;
import com.group6.HaUISocialMedia_server.repository.*;
import com.group6.HaUISocialMedia_server.service.*;
...
public class RelationshipServiceImpl implements RelationshipService {
    ...
    public RelationshipDto sendAddFriendRequest(UUID receiverId) {
        ...
        if (receiver.getId() == requestSender.getId())
            return null;
    ...
    public RelationshipDto acceptFriendRequest(UUID relationshipId) {
        ...
        acFriendMessage
                .setContent(receiver.getLastName() + " " + receiver.getFirstName() + " đã chấp nhận lơi mời kết bạn");
    ...
    public Set<RelationshipDto> getPendingFriendRequests(SearchObject searchObject) {
        ...
        List<Relationship> response = relationshipRepository.findAllPendingRelationship(currentUser.getId(),
    ...
    public Set<RelationshipDto> getSentAddFriendRequests(SearchObject searchObject) {
        ...
        List<Relationship> response = relationshipRepository.findAllSentFriendRequestRelationship(currentUser.getId(),
                PageRequest.of(searchObject.getPageIndex() - 1, searchObject.getPageSize()));
        Set<RelationshipDto> res = new HashSet<>();
        for (Relationship relationship : response) {
            RelationshipDto rela = new RelationshipDto(relationship);
    ...
    public List<UserDto> pagingNewUser(SearchObject searchObject) {
        ...
        String keyword = "";
        if (searchObject.getKeyWord() != null)
            keyword = searchObject.getKeyWord();
        List<User> response = userRepository.findNewUsers(keyword, currentUser.getId(),
                PageRequest.of(searchObject.getPageIndex() - 1, searchObject.getPageSize()));
    ...
    public boolean containsKeyword(String keyword, User user) {
        ...
        if (user.getFirstName().contains(keyword))
            return true;
        if (user.getLastName().contains(keyword))
            return true;
        return false;
    }
    ...
    public RelationshipDto unFriend(UUID relationshipId) {
        ...
        if (entity == null)
            return null;
        // delete notification
        notificationRepository.deleteNotificationAddFriendByIdUser(entity.getReceiver().getId(),
                entity.getRequestSender().getId());
        notificationRepository.deleteNotificationAcceptFriendByIdUser(entity.getRequestSender().getId(),
