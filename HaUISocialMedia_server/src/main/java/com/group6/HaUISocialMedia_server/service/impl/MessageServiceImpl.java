...
import com.group6.HaUISocialMedia_server.repository.MessageRepository;
import com.group6.HaUISocialMedia_server.repository.RoomRepository;
...
public class MessageServiceImpl implements MessageService {
    ...
    public List<MessageDto> findTop20PreviousByMileStone(SearchObject searchObject) {
        ...
        List<MessageDto> data = messageRepository.findTop10ByRoomAndSendDateBeforeOrderBySendDateDesc(
                mileStone.getRoom().getId(), mileStone.getSendDate(), PageRequest.of(0, 20));
    ...
    public boolean isInRoomChat(MessageDto messageDTO) {
        ...
        for (UserRoom userRoom : roomEntity.getUserRooms()) {
            if (userRoom.getUser().getId().equals(currentUser.getId()))
                return true;
        }
    ...
    public MessageDto sendMessage(MessageDto dto) {
        ...
        // send notification for all users that they've received new message in
        // messenger module
        ...
        for (UserDto userDto : userInRooms) {
            ...
            if (userInRooms.size() == 2)
                notification.setContent(notification.getContent() + " cho bạn");
            else
                notification.setContent(notification.getContent() + " đến cuộc hội thoại nhóm");
